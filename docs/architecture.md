...
- **タイトルバードラッグ（ウィンドウ移動）の除外**: エッジ近傍判定を追加した後も「ウィンドウを画面端に持っていくとまだシェルフが出る」（＝ウィンドウ移動とファイルの実際のドラッグを区別できていない）というユーザー報告があった。macOS版（10.1章）の`NSPasteboard(name: .drag)`のようなグローバルなドラッグ内容参照手段がWindowsには無いため、代わりに「左ボタン押下位置がウィンドウのタイトルバー（キャプション領域）だったか」を`WM_NCHITTEST`で判定し、`HTCAPTION`が返った場合はウィンドウ移動の開始とみなして`on_start`を呼ばないようにした（`is_title_bar_hit`）。`WM_NCHITTEST`判定に使う`SendMessageW`は対象ウィンドウへの同期・ブロッキング呼び出しであり、`WH_MOUSE_LL`フックプロシージャ内（低レベルフックタイムアウトの制約下）で行うと危険なため、メッセージループ側の`invoke_on_start`（フックプロシージャの外）でのみ呼び出す設計とした。実機（Windows）での動作確認は未実施
...
- **ドラッグ用パスボードによる判定の追加**: エッジ近傍判定だけでは「ウィンドウを画面端に持っていくとまだシェルフが出る」（＝ウィンドウ移動とファイル/画像/テキストの実際のドラッグを区別できていない）というユーザー報告があったため追加した。`NSPasteboard(name: .drag)`（`NSPasteboardNameDrag`）はOSレベルのドラッグ操作（`NSDraggingSession`、Finderのファイルドラッグ等）が開始されるとデータの型が登録され`changeCount`が増えるが、ウィンドウ移動（タイトルバードラッグ）はこのパスボードに一切触れないため`changeCount`が変わらない。左ボタン押下時点の`changeCount`を基準値として記録し、エッジ近傍判定を通過した後さらに「基準値から変化していて、かつ登録された型が1つ以上ある」ことを条件に追加した（`has_fresh_drag_payload`）。基準値が取得できない場合は安全側に倒し従来通り発火する。この判定が実機のあらゆるアプリのドラッグ操作で確実に機能するかは未検証であり、実機（macOS）での動作確認が必要
